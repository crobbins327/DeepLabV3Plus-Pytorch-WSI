...
# KID_STRUCT_ONNX_PATH = "applikate_kidney_seg/models/KID-seg-resnet34-unet-all_512_10x_v6_LRred-final.onnx"
# KID_STRUCT_PTH_PATH = "applikate_kidney_seg/models/KID-seg-resnet34-unet-all_512_10x_v6_LRred-final.pth"
KID_STRUCT_TF_PATH = "applikate_kidney_seg/models/KID-seg-resnet34-unet-all_512_10x_v6_LRred-final.h5"
...
def get_argparser():
    ...
    parser.add_argument("--batch_size", type=int, required=False, default=32, help="batch size for segmenting patches")
    parser.add_argument("--overlap", type=float, required=False, default=0.0, help="overlap for segmenting patches")
    parser.add_argument("--downsample", type=float, required=False, default=4.0, help="downsample for images")
...
# NOT USED
def postprocess_struct_watershed(
    seg_mask,
    #   roi,
    ...
    ignore_tensor = torch.tensor(ignore_classes, device=seg_mask.device)
    struct_mask = torch.where(~torch.isin(seg_mask, ignore_tensor), 1.0, 0.0)

    k_3x3 = torch.ones(3, 3).to(device)
    k_5x5 = torch.ones(5, 5).to(device)

    # remove smaller dots
    opening = kornia.morphology.erosion(struct_mask, kernel=k_3x3)
...
def contour_classified_mask(segmentation_mask, remove_values=[-1, 0], generate_polygons=True, thickness=1, scale=1.0):
    ...
    for value in classification_values:
        ...
        # for contour in contours:
        cv2.drawContours(contoured_image, contours, -1, RGB_color, thickness)
...
def segment_from_patches(wsi_object, tf_model, batch_size, mask_save_path, decode_fn, overlay_fn, **wsi_kwargs):
    ...
    with tf.device("/GPU:0"):
        for idx, (rois, coord_entries) in enumerate(roi_loader):
            rois_np = rois.cpu().numpy()  # Move to CPU and convert to NumPy
            rois_np = np.transpose(rois_np, (0, 2, 3, 1))  # Convert to channel-last format

            # Convert NumPy array to TensorFlow tensor
            tf_rois = tf.convert_to_tensor(rois_np, dtype=tf.float32)
            # tf model has it's own normalization... so we need to multiply by 255 and undo pytorch normalization...
            batch_mask = tf_model.predict(tf_rois * 255)
            batch_mask_max = torch.from_numpy(np.expand_dims(np.argmax(batch_mask, axis=3), axis=1)).to(device)
            # swap value 4 and 0
            # Whitespace:0, Glom:1, Tub:2, Cap:3, Interstitium:4
            batch_mask_max[batch_mask_max == 4] = 5
            batch_mask_max[batch_mask_max == 0] = 4
            batch_mask_max[batch_mask_max == 5] = 0
            # batch_postproc = postprocess_struct_watershed(batch_mask_max)

            batch_contour_np, contour_list = contour_classified_mask_batch(
                batch_mask_max.squeeze(1).cpu().numpy(), scale=roi_dataset.custom_downsample
            )
            # j = 0
            # j += 1
            # test_contour = batch_contour_np[j]
            # plt.figure(1)
            # plt.imshow(test_contour)
            # plt.figure(2)
            # plt.imshow(rois_np[j])
            # plt.figure(3)
            # plt.imshow(batch_mask_max.squeeze(1).cpu().numpy()[j])
...
def merge_overlapping_polygons(polygons_per_image, coord_entries):
    tree = STRtree([Polygon(polygon.exterior.coords) for polygon, _ in polygons_per_image])
    merged_polygons = []
    for i, (polygon, classif) in enumerate(polygons_per_image):
        # get the coordinates for the polygon
        poly_coords = polygon.exterior.coords
        # get the coordinates for the patch
        ...
        # overlapping_polys = tree.query(poly)
        neighbors = [p for p in tree.query(poly) if p != poly]
        if len(neighbors) == 0:
            merged_polygons.append(poly)
        else:
            # merge the polygons
            merged_poly = unary_union(neighbors)
...
# for QuPath
def create_geojson_result(
    polygons_per_image,
    classif_labels,
    coord_entries,
    geojson_output_path=None,
    object_type="PathDetectionObject",
    lock=False,
):
...
def main():
    ...
    if opts.config_file and os.path.isfile(opts.config_file):
        ...
        patch_args = argparse.Namespace(
            patch_size=opts.patch_size, overlap=opts.overlap, patch_level=0, custom_downsample=opts.downsample
        )
    ...
    def_filter_params = {"a_t": 0.5, "a_h": 0.25, "max_n_holes": 100}
    def_vis_params = {"vis_level": -1, "line_thickness": 75}
    def_patch_params = {"use_padding": True, "contour_fn": "skip"}

    # add def_patch_params to blocky_wsi_kwargs
    blocky_wsi_kwargs.update(def_patch_params)
    ...
    if opts.output is not None:
        ...
        if os.path.isdir(opts.input):
            root_output_dir = os.path.join(opts.input, "output_struct")
        else:
            root_output_dir = os.path.join(os.path.dirname(opts.input), "output_struct")
    ...
    for i in range(len(process_stack)):
        ...
        if seg_params["seg_level"] < 0:
            best_level = wsi_object.find_downsample_level(8)
        ...
        merged_polygons = merge_overlapping_polygons(polygons_per_patch, coord_entries)
        ...
        # Whitespace:0, Glom:1, Tub:2, Cap:3, Interstitium:4
        classif_labels = {
            1: ["Glomeruli", (0, 255, 255)],
            2: ["Tubule", (34, 139, 34)],
            3: ["Capillary", (255, 0, 0)],
            4: ["Interstitium", (255, 255, 0)],
        }
        geojson_output_path = os.path.join(output_dir, slide_id + "_struct_geojson.json")
        geojson_polys = create_geojson_result(
            polygons_per_patch,
            classif_labels,
            coord_entries,
            geojson_output_path=geojson_output_path,
            object_type="PathAnnotationObject",
            lock=True,
        )
