...
def initialize_df(
    slides, seg_params, filter_params, vis_params, patch_params, use_heatmap_args=False, save_patches=False
):
...
def get_simple_loader(dataset, batch_size=1, num_workers=1, collate_fn=collate_features):
    ...
    kwargs = (
        {"num_workers": num_workers, "pin_memory": False, "num_workers": num_workers} if device.type == "cuda" else {}
    )
    loader = DataLoader(
        dataset, batch_size=batch_size, sampler=sampler.SequentialSampler(dataset), collate_fn=collate_fn, **kwargs
    )
    return loader
    ...
    if not testing:
        if training:
            if weighted:
                ...
                loader = DataLoader(
                    split_dataset, batch_size=1, sampler=RandomSampler(split_dataset), collate_fn=collate_MIL, **kwargs
                )
        else:
            loader = DataLoader(
                split_dataset, batch_size=1, sampler=SequentialSampler(split_dataset), collate_fn=collate_MIL, **kwargs
            )

    else:
        ids = np.random.choice(np.arange(len(split_dataset), int(len(split_dataset) * 0.1)), replace=False)
        loader = DataLoader(
            split_dataset, batch_size=1, sampler=SubsetSequentialSampler(ids), collate_fn=collate_MIL, **kwargs
        )
...
def get_optim(model, args):
    if args.opt == "adam":
        ...
        optimizer = optim.SGD(
            filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, momentum=0.9, weight_decay=args.reg
        )
    else:
