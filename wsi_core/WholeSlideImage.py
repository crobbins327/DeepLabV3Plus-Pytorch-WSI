...
from wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, isBlackPatch, isWhitePatch, screen_coords, to_percentiles
...
class WholeSlideImage(object):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
                            filter_params={'a_t':100, 'a_h':16, 'max_n_holes':20}, ref_patch_size=512, exclude_ids=[], keep_ids=[]):
        """
        ...
        img = np.array(self.read_region(x=0, y=0, w=dw, h=dh, level=seg_level,))
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict = self.process_contour(cont, self.holes_tissue[idx], patch_level, save_path, idx, patch_size, step_size, **kwargs)
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, cont_idx, patch_size = 256, step_size = 256,
        ...
        iterable = [(coord, idx, contour_holes, ref_patch_size[0], cont_check_fn) for (coord, idx) in zip(coord_candidates, index_candidates)]
    ...
    def visSegmentationMask(self, mask_hdf5_path=None, vis_level=-1, mask_id="overlay", blank_canvas=False, background_color=(255,255,255)):
