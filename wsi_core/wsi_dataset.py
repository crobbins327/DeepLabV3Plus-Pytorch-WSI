...
def default_transforms(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    t = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
    return t


def get_contour_check_fn(contour_fn="grid_pt", cont=None, ref_patch_size=None, center_shift=None):
    if contour_fn == "skip":
        cont_check_fn = skip_contour_check()
    elif contour_fn == "grid_pt":
        cont_check_fn = isInContourV3_Grid(contour=cont, patch_size=ref_patch_size, grid_size=8)
...
class Wsi_Region(Dataset):
    ...
    args:
        ...
        contour_fn (str):
            ...
            choice of ['four_pt_hard', 'four_pt_easy', 'center', 'basic', 'grid_pt'] (Default: 'grid_pt')
        ...
        **kwargs
        ...
        if coords_hdf5_path is not None and os.path.exists(coords_hdf5_path):
            ...
            for cont_idx, contour in enumerate(wsi_object.contours_tissue):
                ...
                coord_results, contour_meta = wsi_object.process_contour(
                    contour,
                    wsi_object.holes_tissue[cont_idx],
                    level,
                    "",
                    cont_idx,
            ...
            combined_coords = [
                {"coords": coords[i], "grid_ind": grid_indices[i], "cont_idx": filtered_cont_id[i]}
    ...
    def __getitem__(self, idx):
        ...
        read_reg = self.read_region(
            coord[0], coord[1], self.patch_size[0], self.patch_size[0], level=self.desired_level
        )
        patch = np.array(read_reg)
        if self.custom_downsample > 1:
            patch = cv2.resize(patch, self.target_patch_size, self.target_patch_size[1])
