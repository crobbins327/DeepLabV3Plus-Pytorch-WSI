...
def run_nuclei_type_stat(
    ...
    for file_idx, filename in enumerate(file_list[:]):
        ...
        if convert_val_class_dict is not None:
            val_keys = list(convert_val_class_dict.keys())
            for i in range(len(val_keys)):
                true_inst_type[true_inst_type == val_keys[i]] = convert_val_class_dict[val_keys[i]]
        ...
        # dont squeeze, may be 1 instance exist
        pred_centroid = (pred_info["centroid"]).astype("float32")
        pred_inst_type = (pred_info["class"]).astype("int32")

        if consolidate_classes_dict is not None:
            val_keys = list(consolidate_classes_dict.keys())
            for i in range(len(val_keys)):
                pred_inst_type[pred_inst_type == val_keys[i]] = consolidate_classes_dict[val_keys[i]]
                true_inst_type[true_inst_type == val_keys[i]] = consolidate_classes_dict[val_keys[i]]
    ...
    ###
    def _f1_score(paired_true, paired_pred, unpaired_true, unpaired_pred, type_id):
        ...
        precision = tp_dt / (tp_dt + fp_tot) if tp_dt + fp_tot > 0 else 0
        recall = tp_dt / (tp_dt + fn_tot) if tp_dt + fn_tot > 0 else 0
    ...
    for type_uid in type_uid_list:
        ...
        #     w,
        # )
        # type_samples = (paired_true_type == type_uid) | (paired_pred_type == type_uid)
        # f1_type = sklearn.metrics.f1_score(paired_true_type[type_samples], paired_pred_type[type_samples], average="micro")
        f1_type = _f1_score(paired_true_type, paired_pred_type, unpaired_true_type, unpaired_pred_type, type_uid)
...
def run_nuclei_inst_stat(pred_dir, true_dir, print_img_stats=False, ext=".mat"):
    ...
    for filename in file_list[:]:
        ...
        # remove patches with no instance
