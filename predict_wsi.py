...
def postprocess_cell_watershed_v2(seg_mask, 
                                #   roi, 
...
def segment_from_patches(wsi_object, model, batch_size, mask_save_path, decode_fn, overlay_fn, **wsi_kwargs):    
    ...
    with torch.no_grad():
        ...
        for idx, (rois, coord_entries) in enumerate(roi_loader):
            ...
            batch_postproc = postprocess_cell_watershed_v2(batch_mask, 
                                                        #    roi
                                                           )
            ...
            # TODO:
            # convert contour_list of shapely polygons into

            # aggregate classified shapely geometries across WSI

            # # debugging code
            # j=41
            # j=35
            # test_img = (255 * rois[j].cpu().numpy().transpose(1, 2, 0)).astype('uint8')
            # test_mask = batch_postproc.squeeze(1).cpu().numpy()[j] # HW
            # colorized_pred = decode_fn(test_mask).astype('uint8')
            # # colorized_pred = Image.fromarray(colorized_pred)

            # overlay = overlay_fn(test_img, test_mask).astype('uint8')
            # plt.figure(1)
            # plt.imshow(test_img)
            # plt.figure(2)
...
def main():
    ...
    if opts.config_file and os.path.isfile(opts.config_file):
        ...
        #  use default values
        patch_args = argparse.Namespace(patch_size=opts.patch_size, overlap=0, patch_level=0, custom_downsample=1)
    ...
    blocky_wsi_kwargs = {'top_left': None, 'bot_right': None, 'patch_size': patch_size, 'step_size': patch_size, 
                        'custom_downsample':patch_args.custom_downsample, 'level': patch_args.patch_level, 'use_center_shift': False,
                        'transform': transform}

    def_seg_params = {'seg_level': -1, 'sthresh': 15, 'mthresh': 11, 'close': 2, 'use_otsu': False, 
					  'keep_ids': 'none', 'exclude_ids':'none', 'ref_patch_size': patch_args.patch_size}
    ...
    if opts.output is not None:
        ...
        if os.path.isdir(opts.input):
            root_output_dir = os.path.join(opts.input, 'output')
        else:
            root_output_dir = os.path.join(os.path.dirname(opts.input), 'output')
    ...
    for i in range(len(process_stack)):
        ...
        overlay_img = wsi_object.visSegmentationMask(mask_hdf5_path=output_h5_path, vis_level=0, mask_id="overlay", blank_canvas=False, background_color=(255,255,255))
        ...
        geojson_polys = create_geojson_result(polygons_per_patch, classes.classif_labels, coord_entries, geojson_output_path=geojson_output_path)
